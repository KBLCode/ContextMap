...
Monorepo with two self-contained packages. Each package is deployable independently.
...
├── packages/
│   │
│   ├── opencode/                      # OpenCode plugin (self-contained)
│   │   ├── package.json
│   │   ├── tsconfig.json
│   │   ├── README.md                  # OpenCode-specific install instructions
│   │   ├── plugin/                    # Ready to copy to ~/.config/opencode/plugins/
│   │   │   ├── context-viewer.mjs     # Main plugin file (bundled)
│   │   │   └── package.json           # Plugin dependencies
│   │   └── src/
│   │       ├── index.ts               # Plugin entry point
│   │       └── cmap-tool.ts           # /cmap custom tool
│   │
│   └── claude-code/                   # Claude Code plugin (self-contained)
│       ├── README.md                  # Claude Code-specific install instructions
│       └── context-viewer/            # Ready to copy to project or ~/.claude/
│           ├── .claude-plugin/
│           │   └── plugin.json        # Plugin manifest
│           ├── commands/
│           │   └── cmap.md            # /cmap slash command
│           ├── hooks/
│           │   └── hooks.json         # Token tracking hooks
│           └── scripts/
│               └── track-tokens.js    # Hook script (bundled core)
│
└── scripts/
    ├── build.sh                       # Build all packages
    └── bundle.sh                      # Bundle core into each package
```

### Package Independence

Each package in `packages/` is **fully self-contained**:

- **OpenCode**: Copy `packages/opencode/plugin/` to `~/.config/opencode/plugins/context-viewer/`
- **Claude Code**: Copy `packages/claude-code/context-viewer/` to your project or `~/.claude/`

The `core/` library is bundled INTO each package at build time - no external dependencies needed at runtime.
